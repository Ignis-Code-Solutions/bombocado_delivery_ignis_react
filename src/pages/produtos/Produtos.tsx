...
function Produtos() {
  ...
  if (!authContext) {
    throw new Error("Produtos deve ser utilizado dentro de um AuthProvider.");
  }

  const { usuario, token, isAuthenticated, logout } = authContext;
  ...
  const produtoSalvo = Boolean(state?.produtoSalvo);

  const produtoExcluido = Boolean(state?.produtoExcluido);
  ...
  const carregarProdutos = useCallback(async () => {
    ...
    try {
      ...
      const resposta = await buscar<Produto[]>("/produtos", token);

      // FILTRO INVERSO: Remove as doações e sacolas surpresas da vitrine principal
      const produtosNormais = resposta.filter((produto) => {
        const nomeCategoria = (produto.categoria?.nome || "").toLowerCase();
        return (
          !nomeCategoria.includes("doação") &&
          !nomeCategoria.includes("solidári") &&
          !nomeCategoria.includes("surpresa")
        );
      });

      // Salva no estado apenas os produtos normais
  ...
  useEffect(() => {
    ...
    void carregar();
  }, [isAuthenticated, carregarProdutos, navigate]);
  ...
  const produtosFiltrados = useMemo(() => {
    ...
    return produtos.filter((produto) => {
      ...
      const categoria = produto.categoria?.nome?.toLocaleLowerCase("pt-BR");
    ...
    <main className="min-h-screen bg-surface">
      <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        {produtoSalvo && (
          ...
          >
            Produto {state?.acao === "editado" ? "editado" : "cadastrado"} com
            sucesso!
          </div>
        )}
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-3 text-sm leading-6 text-ink-soft sm:text-base">
                Encontre alimentos disponíveis na plataforma e descubra opções
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {!carregando && !erro && (
              <p className="shrink-0 text-sm text-ink-soft">
                <strong className="text-ink">{produtosFiltrados.length}</strong>{" "}
        ...
        {!carregando && erro && (
          <section className="mt-6 rounded-card bg-white p-8 text-center shadow-card sm:p-10">
            ...
            <h2 className="mt-5 text-2xl">Não foi possível carregar</h2>
        ...
        {!carregando && !erro && produtosFiltrados.length === 0 && (
          <section className="mt-6 rounded-card bg-white p-8 text-center shadow-card sm:p-10">
            ...
            <h2 className="mt-5 text-2xl">Nenhum produto encontrado</h2>
                ...
                >
                  {produto.imagem ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center text-ink-muted">
                      <PackageIcon size={46} weight="duotone" />
                    </div>
                  )}
                  ...
                  <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-ink-muted">
                    {produto.tempoEntrega !== null && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>{produto.tempoEntrega} min</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>{formatarData(produto.dataValidade)}</span>
                    </div>
                  </div>
